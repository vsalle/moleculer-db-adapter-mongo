...
class MongoDbAdapter {
    ...
    createCursor(params, isCounting) {
        if (params) {
            let q;
            if (isCounting) q = this.collection.countDocuments(params.query);
            else q = this.collection.find(params.query);
            // Full-text search
        ...
        // If not params
        if (isCounting) return this.collection.countDocuments({});
        else return this.collection.find({});
    }
     ...
     */
    stringToObjectID(id) {
        if (typeof id == 'string' && ObjectID.isValid(id)) return new ObjectID.createFromHexString(id);
